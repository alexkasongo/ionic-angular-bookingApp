...
/* ######### UTILITY FUNCTION: Convert string to file ##############*/
...
export class NewOfferPage implements OnInit {
  ...
  onImagePicked(imageData: string | File) {
    ...
    if (typeof imageData === 'string') {
      // try block so that incase this it fails then we can catch the error and do something
      try {
  ...
  onCreatedOffer() {
    ...
    this.loaderCtrl.create({
      ...
      loadingEl.present();
      this.placeService.uploadImage(this.form.get('image').value).pipe(switchMap(uploadRes => {
